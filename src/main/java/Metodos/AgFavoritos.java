...
public class AgFavoritos {
     ...
     public boolean FavoritosEl(JLabel label) {
          Connection conn = null;
          PreparedStatement stmt = null;
          boolean correcto = false;

          String Insert = "DELETE FROM favoritos WHERE idpokemon = '"+label.getText()+"'";
          ...
          try {
               ...
               JOptionPane.showMessageDialog(null, ex + "No se elimino a favoritos");
          } finally {
               Conexion.CloseConection(stmt);
               Conexion.CloseConection(conn);
